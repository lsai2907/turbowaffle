...
ggplot(data = world) +
  ...
  labs(title = element_blank(),
       ...
       fill = element_blank()) + # Changed the legend title
  coord_sf() + 
  coord_sf(
    xlim = c(-130, -60),  # Longitude range (Western Hemisphere)
    ylim = c(17, 50)       # Latitude range (Northern Hemisphere only)
  ) +
  theme_minimal() +
  theme(
    text = element_text(family = "serif"),
    axis.title = element_text(size = 30, face = "bold"),
    axis.text = element_text(size = 20, color = "black"),   # Apply font globally
...
ggplot(data = world) +
  ...
  coord_sf() + 
  coord_sf(
    xlim = c(-170, -142),  # Longitude range (Western Hemisphere)
    ylim = c(50, 72)       # Latitude range (Northern Hemisphere only)
  ) +
  theme_minimal() +
  theme(
    text = element_text(family = "serif"),  
    ...
    legend.text = element_text(size = 20, face = "bold"),
    legend.position = "bottom",
    aspect.ratio = 1    # <- FORCE 1:1 aspect ratio (perfect circles)
  )
