...
# Get US states map as an sf object
world <- st_as_sf(map("state", plot = FALSE, fill = TRUE))
...
# Load US states map as sf object
world <- st_as_sf(map("state", plot = FALSE, fill = TRUE))

# Separate Alaska
alaska <- pie_data[pie_data$Longitude < -140 & pie_data$Latitude > 50, ]
continental <- pie_data[!(pie_data$Longitude < -140 & pie_data$Latitude > 50), ]

# Manually shift Alaska’s coordinates
# Move it right and down
alaska$Longitude <- alaska$Longitude + 50  # shift east
alaska$Latitude <- alaska$Latitude - 25    # shift south

# Combine adjusted data
pie_data_adj <- rbind(continental, alaska)

# Plot
ggplot(data = world) +
  geom_sf(fill = "grey", color = "white") +
  
  # Pie charts (including shifted Alaska)
  geom_scatterpie(data = pie_data_adj,
                  aes(x = Longitude, y = Latitude, group = region),
                  cols = c("Archaea", "Bacteria", "Eukarya", "Unclassified"),
                  pie_scale = 0.75) +
  
  geom_text(data = pie_data_adj,
            aes(x = Longitude, y = Latitude, label = region),
            size = 1.75, vjust = -3, family = "serif") +
  
  coord_sf(xlim = c(-130, -60), ylim = c(20, 52), expand = FALSE) +
  labs(
    title = "Microbial Domain Abundance Across US Sites",
    subtitle = "With Alaska repositioned for clarity",
    fill = "Domain"
  ) +
  theme_minimal() +
  theme(
